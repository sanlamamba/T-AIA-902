...
class TaxiDriver:
    ...
    def _print_results(self, result):
        ...
        print(f"{'─'*50}")
        print(f"Mean Reward: {result['mean_reward']:.2f} ± {result.get('std_reward', 0):.2f}")
        print(f"Mean Steps: {result['mean_steps']:.1f} ± {result.get('std_steps', 0):.1f}")
    ...
    def _print_benchmark_summary(self, results):
        ...
        print(f"🥇 Best Average Reward: {best_reward['algorithm']} ({best_reward['mean_reward']:.2f})")
        print(f"🥇 Best Efficiency: {best_efficiency['algorithm']} ({best_efficiency.get('efficiency_score', 0):.4f})")
        print(f"🥇 Best Win Rate: {best_winrate['algorithm']} ({best_winrate['win_rate']:.2%})")
        ...
        for result in results:
            if result['algorithm'] != 'BruteForce':
                reward_improvement = ((result['mean_reward'] - bf_result['mean_reward']) / abs(bf_result['mean_reward'])) * 100
                step_improvement = ((bf_result['mean_steps'] - result['mean_steps']) / bf_result['mean_steps']) * 100
                print(f"{result['algorithm']:12} | Reward: {reward_improvement:+6.1f}% | Steps: {step_improvement:+6.1f}%")
...
def main():
    ...
    parser.add_argument("--mode", choices=["user", "time-limited", "benchmark"], 
                       default="benchmark", help="Mode to run")
    parser.add_argument("--algorithm", choices=["bruteforce", "q-learning", "sarsa", "dqn"], 
                       default="q-learning", help="Algorithm to use")
    ...
    if args.mode == "user":
        ...
        driver.user_mode(args.algorithm, args.train_episodes, args.test_episodes, **params)
